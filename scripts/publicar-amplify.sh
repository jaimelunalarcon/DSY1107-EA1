...
for h in aws zip curl python3; do
  command -v "$h" >/dev/null 2>&1 || { echo "Falta '$h' en el PATH." >&2; exit 1; }
done

# Se valida lo que se va a publicar, no lo que se cree que se compiló.
[[ -f "$DIRECTORIO/index.html" ]] \
  || { echo "No existe $DIRECTORIO/index.html" >&2; exit 1; }
[[ -f "$DIRECTORIO/config.json" ]] \
  || { echo "config.json no llegó al bundle (debe estar en frontend/public/)" >&2; exit 1; }

TMP_DIR="$(mktemp -d)"
ZIP="$TMP_DIR/amplify-dist.zip"
limpiar() { rm -rf "$TMP_DIR"; }
trap limpiar EXIT

# Se comprime el CONTENIDO del directorio: index.html tiene que quedar en la
# raíz del zip o Amplify sirve un 404. El .example no se publica.
...
job="$(python3 -c "import json,sys; print(json.load(sys.stdin)['jobId'])" <<< "$despliegue")"
subida="$(python3 -c "import json,sys; print(json.load(sys.stdin)['zipUploadUrl'])" <<< "$despliegue")"
...
for _ in $(seq 1 "$INTENTOS"); do
  ...
  case "$estado" in
    SUCCEED)          break ;;
    FAILED|CANCELLED) echo "El despliegue terminó en $estado (job $job)" >&2; exit 1 ;;
  esac
  sleep "$ESPERA"
done
